...
6. ~~**Resend.**~~ Done 2026-09-14: `fromthecall.com` verified at Resend and Supabase SMTP applied from the repo. Sign-up and reset mail now goes out through Resend rather than Supabase's team-only mailer, at 30 an hour rather than 2. Original note: Custom SMTP is what actually opens signup to the world: Supabase's own mailer only delivers to your project team. The code side is done — `node scripts/supabase-auth-config.mjs --apply` configures it from `RESEND_API_KEY`. What is left is yours: verify `fromthecall.com` at resend.com, create a sending key, and put it in `.env.local` **and** Vercel. The "usage cap" this item used to demand was dropped deliberately (2026-09-14): a cap punishes the customer paying you. A loop guard was built instead, and the real spend backstop belongs in the Anthropic and Deepgram consoles.
7. **Stripe.** Live keys are in Vercel and checkout reaches Stripe (2026-09-16). **Still unproven: the webhook**, which is the only thing that moves an account to Pro. A wrong or other-mode `STRIPE_WEBHOOK_SECRET` fails silently from the customer's side — the card is charged and the account stays Free. Check the endpoint's delivery log in Stripe for 200s, then prove the whole path without a card: a live 100%-off promotion code applied at checkout, the account flips to Pro, then delete the code. New accounts are now offered Pro once on `/welcome`; the pricing band's buttons still start on Free, deliberately.
