...
## Conventions
- **Reasoning effort.** Drafting (`src/lib/agent.ts`) sets `output_config.effort` to `DRAFT_EFFORT` ("low"): reasoning tokens bill as output, and a draft is a short rephrasing a person approves before anything is sent. Extraction deliberately sets **nothing** and runs at the model's default. `npm run compare:effort [meetingId] --yes` re-runs a real transcript at several levels and reports what each one caught, not just what it cost; it is read-only against the database and refuses to call the API without `--yes`. Measured on two short transcripts: low saved nothing (it used *more* output tokens than medium on one of them) and dropped the owner from every action item on the other. Do not move extraction to low without re-running this on a long transcript and reading the diff.
