...
- **The rail is ink, the work is paper.** `.rail` is near-black beside the warm page, the same alternation the shopfront runs down the screen, and it keeps navigation out of the way of the three hues that mean something inside the content. One `.rail-pill` slides between the tabs; `sidebar.tsx` measures the active link and writes `transform`, `width` and `height` straight to the node rather than holding them in state, so a render never fights the transition. The first placement disables the transition so it does not slide in from the corner.
- **The app page has a grain**, a barely-there dot grid on `body.app`, and cards carry a two-part shadow. Together they are what stop a flat light UI reading as unfinished; the marketing surface has neither.
