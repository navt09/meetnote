...
## UI
- **The app is one ruled sheet on a desk, not a stack of cards.** `.sheet` (in `layout.tsx`, wrapping `main` and the footer) is the `--panel` column with a hairline down each side running the full height, sitting on the deeper `--bg` desk. Sections are separated by rules, never by gaps between floating boxes: put them in a `.ledger` and each gets a line above it. `.bleed` pushes a rule back out past the reading inset to the sheet's edge, which is how `PageHead` gets a title rule that spans the column. Nothing on paper floats, so **nothing in the app has a shadow**, corners are 8px, and a hairline does the work a radius used to. An earlier version had a dot-grid background and two-part card shadows; both were removed as generic.
...
- **The rail is ink, the work is paper.** `.rail` is near-black beside the sheet, the same alternation the shopfront runs down the screen, and it keeps navigation out of the way of the three hues that mean something inside the content. One `.rail-pill` slides between the tabs; `sidebar.tsx` measures the active link and writes `transform`, `width` and `height` straight to the node rather than holding them in state, so a render never fights the transition. The first placement disables the transition so it does not slide in from the corner.
