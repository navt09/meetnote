...
export async function POST(req: Request, ctx: Ctx) {
  ...
  if (!process.env.ANTHROPIC_API_KEY)
    return NextResponse.json(
      { error: "Drafting isn't configured." },
      { status: 500 },
    );
  ...
  if (meetingError) {
    console.error(
      JSON.stringify({
        event: "draft_email_lookup_error",
        id,
        message: meetingError.message,
      }),
    );
    return NextResponse.json(
      { error: "Could not load that meeting." },
      { status: 500 },
    );
  }
  if (!meetingData)
  ...
  const meeting = meetingData as Pick<
    Meeting,
    "id" | "title" | "notes" | "transcript"
  >;

  // Only somebody the notes actually named; no free-text targets.
  const person = meeting.notes?.people_to_contact?.find(
    (p) => p.name.toLowerCase() === name.toLowerCase(),
  );
  if (!person)
    return NextResponse.json(
      { error: "That person isn't in this meeting's notes." },
      { status: 400 },
    );
  ...
  // Upsert, not insert: drafting the same person for the same meeting twice
  // replaces the draft rather than stacking a second one in the queue, which
  // is the rule tickets already follow per task. Reachable from two pages now,
  // so it happens.
  const { data, error } = await auth.db
    ...
    .upsert(
      ...
      { onConflict: "meeting_id,recipient" },
    )
  ...
  if (error || !data) {
    console.error(
      JSON.stringify({
        event: "draft_email_save_error",
        id,
        message: error?.message,
      }),
    );
    return NextResponse.json(
      { error: "Drafted it, but couldn't save it. Try again." },
      { status: 500 },
    );
  }
  ...
  return NextResponse.json(
    { draft: toPublicDraft(row, row.meetings?.title ?? meeting.title) },
    { status: 201 },
  );
}
