...
export async function GET(req: Request, ctx: Ctx) {
  ...
  if (error) {
    console.error(JSON.stringify({ event: "meeting_get_error", id, message: error.message }));
...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  if (error) {
    console.error(JSON.stringify({ event: "meeting_rename_error", id, message: error.message }));
    return NextResponse.json({ error: "Could not rename this meeting." }, { status: 500 });
  }
  if (!data) return NextResponse.json({ error: "Meeting not found" }, { status: 404 });
...
export async function DELETE(req: Request, ctx: Ctx) {
  ...
  if (error) {
    console.error(JSON.stringify({ event: "meeting_delete_lookup_error", id, message: error.message }));
    return NextResponse.json({ error: "Could not delete this meeting." }, { status: 500 });
  }
  ...
  if (path && pathBelongsTo(path, auth.user.id)) {
    ...
    if (rmError && !/not found/i.test(rmError.message)) {
      console.error(JSON.stringify({ event: "meeting_audio_delete_error", id, message: rmError.message }));
      return NextResponse.json({ error: "Could not delete the audio. Try again in a minute." }, { status: 502 });
    }
  }

  const { error: delError } = await auth.db.from("meetings").delete().eq("id", id);
  if (delError) {
    console.error(JSON.stringify({ event: "meeting_delete_error", id, message: delError.message }));
    return NextResponse.json({ error: "Could not delete this meeting." }, { status: 500 });
  }
