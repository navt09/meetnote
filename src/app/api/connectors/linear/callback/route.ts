...
export async function GET(req: Request) {
  ...
  if (!(await consumeState("linear", url.searchParams.get("state")))) {
    return NextResponse.redirect(backToSettings(origin, { error: "That Linear connection attempt expired. Try again." }));
  }

  const denied = url.searchParams.get("error");
  if (denied) {
    return NextResponse.redirect(
      backToSettings(origin, { error: denied === "access_denied" ? "You declined the Linear permissions." : "Linear reported a problem." }),
    );
  }

  const code = url.searchParams.get("code");
  if (!code) return NextResponse.redirect(backToSettings(origin, { error: "Linear didn't return an authorisation code." }));
  ...
  try {
    ...
    if (teams.length === 0) {
      return NextResponse.redirect(backToSettings(origin, { error: "Linear connected, but the account can't see any teams." }));
    }
    ...
    return NextResponse.redirect(
      backToSettings(origin, {
        notice: config.teamName ? `Linear connected. Issues will go to ${config.teamName}.` : "Linear connected. Pick a team.",
      }),
    );
  } catch (err) {
    console.error(JSON.stringify({ event: "linear_callback_error", message: err instanceof Error ? err.message : String(err) }));
    return NextResponse.redirect(backToSettings(origin, { error: err instanceof Error ? err.message : "Couldn't finish connecting Linear." }));
  }
}
